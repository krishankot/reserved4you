...
{   
            ...
            if (!empty($user)) {                
                ...
                if ($role->name=='Service Provider') {
                    if (Hash::check($request['password'], $user->password)) {
						$dataUpdate = array();
						$device_token = $device_id  = $device_type = NULL;
						if(!empty($request['device_token'])){
							$device_token = $dataUpdate['device_token'] = $request['device_token'];
						}
						if(!empty($request['device_id'])){
							$device_id = $dataUpdate['device_id'] = $request['device_id'];
							$ApiSession  = ApiSession::where('user_id', $user->id)->where('device_id', $device_id)->first();
						}else{
							$ApiSession  = ApiSession::where('user_id', $user->id)->first();
						}
						if(!empty($request['device_type'])){
							$device_type = $dataUpdate['device_type'] = $request['device_type'];
						}
						
						if(!empty($ApiSession->session_id)){
							$token = $ApiSession->session_id;
						}else{
							$token = \BaseFunction::setSessionData($user->id, $device_id, $device_token, $device_type);
						}
    ...
    public function logout(Request $request)
    {
		...
		if(!empty($request['device_id'])){
			ApiSession::where('user_id', $user_id)->where('device_id', $request['device_id'])->delete();
		}else{
