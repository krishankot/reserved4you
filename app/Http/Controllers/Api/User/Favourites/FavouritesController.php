...
class FavouritesController extends Controller
{
    public function list(Request $request)
    {
        try {
            ...
            $favoritList = Favourite::with('StoreDetail','StoreDetail.storeCategory','StoreDetail.storeCategory.CategoryData','StoreDetail.storeRated','StoreDetail.storeGallery')
                                    ->where('user_id',$userId)
                                    ->select('id','user_id','store_id')
                                    ->get();
            
            foreach ($favoritList as $value) {
				$value['is_value']   = $value['StoreDetail']['is_value'];
                ...
                $value['category_name']   = "Kosmetik"; //$value['StoreDetail']['category_id'];
                //category name
                $category = [];
                foreach ($value->StoreDetail['storeCategory'] as $key) {                    
                    $category[]['name']= $key['CategoryData']['name'];                       
                }                
                $value['store_category'] = $category;
                //store rating
                // $sum = 0;    
                ...
                $value['total_rating']    = number_format($sum,1);   
                $value['avg_rating']      = number_format($sum == 0 ? 0 : ($sum / $value->StoreDetail['storeRated']->count()),1);
                $value['total_feedback']  = $value->StoreDetail['storeRated']->count();                   
                $value['discountFlage']=\BaseFunction::checkDiscountForStore($value->store_id);            
                $value['discount']=number_format(\BaseFunction::findStoreDiscount($value->store_id),0);                       
                $value['store_gallery']   = $value->StoreDetail['storeGallery'];
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['ResponseCode' => 1, 'ResponseText' => __('The store has been added to the favorites list'),'ResponseData'=> $storeFavorites],200);
