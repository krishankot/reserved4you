    ...
    <div class="row page-title">
        <div class="col-md-12">
            <nav aria-label="breadcrumb" class="float-right">
                <a href="{{URL::to('master-admin/features/create')}}" class="btn btn-primary text-white">+ Eigenschaft erstellen </a>
            </nav>
            <h4 class="mb-1 mt-1"> Eigenschaften</h4>
        </div>
    </div>
                    ...
                    <table id="basic-datatable" class="table dt-responsive nowrap">
                        ...
                        @foreach($data as $row)
                            <tr>
                                ...
                                <td>{{$row->name}}</td>
                                <td>
                                    @if(file_exists(storage_path('app/public/features/'.$row->image)) && $row->image != '')
                                        <img src="{{URL::to('storage/app/public/features/'.$row->image)}}"
                                ...
                                <td>
                                    <a class="" href="{{URL::to('master-admin/features/'.$row->id.'/edit')}}"> <i
                                            class="uil-pen"></i></a>
                                    <a class="" href="{{URL::to('master-admin/features/'.$row->id.'/destroy')}}"><i
