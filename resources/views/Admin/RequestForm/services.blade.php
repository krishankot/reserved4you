		...
		<div class="row">
			<div class="col-lg-12">
				<div class="form-group">
				   ...
				   {{Form::select('service['.$i.'][category_id]',[''=>__('Select Category')] + $categories,!empty($dataRequest['service'][$i]['category_id'])?$dataRequest['service'][$i]['category_id']:NULL,array('data-id' => $i, 'id' => 'select'.$i, 'class'=>'select select-time category_select'))}}
				...
				<div class="form-group">
					...
					<div class="image-box" rel="imgUpload{{$i}}" id="image_drop_area{{$i}}">
						...
						<label id="file_name{{$i}}" for="imgUpload{{$i}}" class="d-flex align-items-center" >
							...
							@if(!empty($dataRequest['service'][$i]['imagename']))
								<a  href="{{ route('admin_download', array($dataRequest['service'][$i]['imagename'], 'requestFormTemp')) }}" class="btn btn-pink btn-photo ml-5">
									{{__('Download') }}
								</a>
							@endif
						</label>
				...
				<div id="div_subservices{{$i}}">
					@if(!empty($dataRequest['service'][$i]['sub_service']))
						@foreach($dataRequest['service'][$i]['sub_service'] as $s=>$item)
							@include('Admin/RequestForm/subservices', ['i' =>$i, 's' => $s])
						@endforeach
					@else
						@include('Admin/RequestForm/subservices', ['i' =>$i, 's' => 1])
					@endif
				</div>
			</div>
		</div>
