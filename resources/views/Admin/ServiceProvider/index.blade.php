    ...
    <div class="row page-title">
        <div class="col-md-12">
            <nav aria-label="breadcrumb" class="float-right">
                <a href="{{URL::to('master-admin/service-provider/create')}}" class="btn btn-primary text-white">+ Dienstleister erstellen</a>
            </nav>
            <h4 class="mb-1 mt-1"> Dienstleister</h4>
        </div>
    </div>
                    ...
                    <table id="basic-datatable" class="table dt-responsive nowrap">
                        ...
                        @foreach($data as $row)
                            <tr>
                                ...
                                <td>
                                    <a href="{{URL::to('master-admin/service-provider/'.$row->id.'/status')}}"> <label
                                            class="badge badge-soft-{{$row->status == 'active' ? 'success' : 'danger'}}">{{$row->status}}</label>
                                    </a>
                                </td>
                                <td>
                                    <a class="" href="{{URL::to('master-admin/service-provider/'.$row->id.'/edit')}}"> <i
                                            class="uil-pen"></i></a>
                                    <a class="" href="{{URL::to('master-admin/service-provider/'.$row->id.'/destroy')}}"><i
