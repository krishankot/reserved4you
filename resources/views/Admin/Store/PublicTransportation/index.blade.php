    ...
    <div class="row page-title">
        <div class="col-md-12">
            <nav aria-label="breadcrumb" class="float-right">
                <a href="{{URL::to('master-admin/store-profile/'.$id.'/public-transportation/create')}}" class="btn btn-primary text-white">+ Create Public Transporatation</a>
            </nav>
            <h4 class="mb-1 mt-1">Store Public Transporatation</h4>
        </div>
    </div>
                    ...
                    <table id="basic-datatable" class="table dt-responsive nowrap">
                        ...
                        @foreach($data as $row)
                            <tr>
                                <td>{{$row->id}}</td>
                                <td>{{$row->title}}</td>
                                <td>{{$row->transportation_no}}</td>
                                <td>
                                    @if(file_exists(storage_path('app/public/store/transportation/'.$row->image)) && $row->image != '')
                                        <img src="{{URL::to('storage/app/public/store/transportation/'.$row->image)}}"
                                ...
                                <td>
                                    <a class="" href="{{URL::to('master-admin/store-profile/'.$id.'/public-transportation/'.$row->id.'/edit')}}"> <i
                                            class="uil-pen"></i></a>
                                    <a class="" href="{{URL::to('master-admin/store-profile/'.$id.'/public-transportation/'.$row->id.'/destroy')}}"><i
