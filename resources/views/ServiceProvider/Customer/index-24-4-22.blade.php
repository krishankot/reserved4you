...
{{--                </select>--}}
			  ...
			  <a href="{{route('downloadSample')}}" class="appointment-btn btn-yellow ">{{ __('Download Sample') }}</a>
			   {{Form::open(array('url'=>route('importCustomers'),'method'=>'post','name'=>'customerlistForm','id'=>'customerlistForm','files'=>'true'))}}
					@csrf
					 
					<input id="customerlist" accept=".csv" type="file" name="customerlist" hidden />
					<label for="customerlist"  class="appointment-btn btn-yellow ml-1 mb-0 mr-0">{{ __('Import Customers') }}</label>
					
				{{Form::close()}}
			
            <a href="{{URL::to('dienstleister/kunden-hinzufuegen')}}" class="appointment-btn btn-yellow  ml-1">Neuen Kunden hinzufügen </a>
			 ...
			 $.ajax({  
				...
				success: function(data) { 
					if(data.ResponseCode == 1){
						window.location.href = "{{ url('dienstleister/kunden') }}"
					}
				}
			 });
		});
