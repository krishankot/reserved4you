        ...
        <div class="withdraw-now">
            ...
            <div class="withdraw">
                <div class="available-bal">
                    ...
                    <h6>{{number_format($availableBalance,2,',','.')}}€</h6>
                </div>
                {{-- <a href="#" class="btn-yellow">Withdrow Now</a> --}}
     ...
     <div class="wallet">
        <div class="dashboard-row wallet-row">
            ...
            <div class="dashboard-box  light-pink-box">
                        <span>
                            <img src="{{URL::to('storage/app/public/Serviceassets/images/icon/euro.svg')}}" alt="">
                        </span>
                <p>Heutige Einnahmen</p>
                <h5>{{number_format($todayEarning,2, ',','.')}}€</h5>
            ...
            <div class="dashboard-box light-blue-box">
                        <span>
                            <img src="{{URL::to('storage/app/public/Serviceassets/images/icon/euro-money.svg')}}"
                                 alt="">
                        </span>
                <p>
                    Gesamt <br />
                ...
                <h5>{{number_format($netEarning,2,',','.')}}€</h5>
            ...
            <div class="dashboard-box light-green-box">
                        <span>
                            <img src="{{URL::to('storage/app/public/Serviceassets/images/icon/get-money.svg')}}" alt="">
                        </span>
                <p>Verfügbar <br />(Onlinezahlungen)</p>
                <h5>{{number_format($availableBalance,2,',','.')}}€</h5>
            ...
            <div class="dashboard-box light-purple-box">
                        <span>
                            <img src="{{URL::to('storage/app/public/Serviceassets/images/icon/withdrawal.svg')}}"
                                 alt="">
                        </span>
                <p>Auszahlung <br />(Onlinezahlungen)</p>
                <h5>{{number_format($withdrwalBalance,2,',','.')}}€</h5>
            ...
            <div class="dashboard-box light-orange-box">
                        <span>
                            <img src="{{URL::to('storage/app/public/Serviceassets/images/icon/refund.svg')}}" alt="">
                        </span>
                <p>Rückerstattung</p>
                <h5>{{number_format($refundableBalance,2,',','.')}}€</h5>
		  ...
		  <div class="transaction justify-content-end  mt-3">
				<ul class="nav nav-pills period-pills transac-tab" id="pills-tab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <a class="nav-link period-tab {{ ($period == 'today')?'active':'' }}" data-value="today" data-toggle="pill" href="#today" role="tab" aria-selected="false">heute</a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link period-tab {{ ($period == 'month' or $period == '')?'active':'' }}" data-value="month" data-toggle="pill" href="#month" role="tab" aria-selected="false">diesen Monat</a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link period-tab {{ ($period == 'year')?'active':'' }}" data-value="year" data-toggle="pill" href="#year" role="tab" aria-selected="false">Jahr</a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a class="nav-link {{ ($period == 'custom')?'active':'' }}" id="daterange" data-value="custom" data-toggle="pill" href="#custom" role="tab" aria-selected="true">Benutzerdefiniert</a>
                    </li>
                </ul>
			</div>
			<div class="d-flex align-items-center justify-content-end">
				 {{Form::open(array('url'=>"dienstleister/finanzen",'method'=>'get','name'=>'change_month','id'=>'change_month'))}}
					{{Form::hidden('period', null ,array('class'=>'selected_period'))}}
					{{Form::hidden('start_date', null ,array('class'=>'startdate'))}}
					{{Form::hidden('end_date', null ,array('class'=>'enddate'))}}
                {{Form::close()}}
			</div>
        ...
        <div class="tab-content owl-buttons" id="pills-tabContent">
            <div class="tab-pane fade active show" id="transaction" role="tabpanel"
                ...
                <div class="transaction-heading">
                    <p>
                        <span> {{number_format(count($transaction),0)}} </span> Transaktionen 
                    </p>
                </div>
                    ...
                    <table id="example" class="table table-striped table-bordered  nowrap customers-table"
                        ...
                        @foreach($transaction as $row)
                            <tr>
                                <td>
                                    ...
                                    <span>
                                        @if(file_exists(storage_path('app/public/user/'.\BaseFunction::getUserProfile($row['email']))) && \BaseFunction::getUserProfile($row['email']) != '')
                                            <img
                                                src="{{URL::to('storage/app/public/user/'.\BaseFunction::getUserProfile($row['email']))}}"
                                                alt="user">
                                        @else
                                            <img
                                ...
                                <td data-order="{{$row['order_id']}}">#{{$row['order_id']}}</td>
								<td>#{{$row['payment_id'] == 'Cash' ? 'vor Ort' : $row->payment_id}}</td>
                                <td>{{\Carbon\Carbon::parse($row['created_at'])->translatedFormat('d M, Y (D) H:i')}}</td>
                               <td class="status">
								<span class="{{$row['payment_status'] == 'succeeded' ?'completed-label2' :($row['payment_status'] == 'failed' ? 'cancel-label2':($row->status == 'cancel'?'pending-label2':'completed-label2'))}}">{{$row->payment_status == 'succeeded' ? 'erfolgreich' : ($row->payment_status == 'failed' ? 'fehlgeschlagen' :($row->status == 'cancel'?'rückerstattet ':'erfolgreich '))}}</span>
							</td>
                                <td>{{number_format(str_replace(",", '', $row['total_amount']),2,',','.')}}€</td>
                                <td>
                                    <div class="tabel-action">
                                        <a class="more-detail-link" onclick="redirectMoreDetails({{$row['id']}}, '{{encrypt($row['appointment_id'])}}');" href="javascript:void(0);">Mehr Details</a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
            ...
            <div class="tab-pane fade  show" id="refund" role="tabpanel" aria-labelledby="pills-refunds-tab">
                <div class="transaction-heading">
                    <p>
                        <span>  {{number_format(count($refund),0)}}  </span> Rückerstattung
                    </p>
                </div>
                ...
                <table id="example1" class="table table-striped table-bordered  nowrap customers-table"
                    ...
                    @foreach($refund as $row)
                        <tr>
                            ...
                            <td>{{\Carbon\Carbon::parse($row['created_at'])->translatedFormat('d M, Y (D) H:i')}}</td>
                            <td>
                            {{\Carbon\Carbon::parse($row['updated_at'])->translatedFormat('d M, Y (D) H:i')}}</td>
                            <td>{{number_format($row['price'],2,',','.')}}€</td>
                            <td>
                                <div class="tabel-action">
                                    <a class="more-detail-link"
                                       onclick="redirectMoreDetails({{$row['id']}}, '{{encrypt($row['appointment_id'])}}');" href="javascript:void(0);">Mehr Details</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
            ...
            <div class="tab-pane fade  show" id="withdraw" role="tabpanel" aria-labelledby="pills-withdraw-tab">
                <div class="transaction-heading">
                    <p>
                        <span> {{number_format(count($withdraw),0)}} </span> Auszahlung
                    </p>
                </div>
...
@endsection
@section('service_js')
    <script src="{{URL::to('storage/app/public/Serviceassets/js/jquery.md5.min.js')}}"></script>
...
<script type="text/javascript" src="//cdn.jsdelivr.net/momentjs/latest/moment-with-locales.min.js"></script>
   <script type="text/javascript" src="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js"></script>
    <script>
		...
		var maxDate = "{{ date('d-m-Y') }}";
		moment.locale('de');
		@if($startdate && $enddate)
			var startdateOld = "{{ date('d-m-Y', strtotime($startdate)) }}";
			var enddateOld = "{{ date('d-m-Y', strtotime($enddate)) }}";
		...
		function redirectMoreDetails(id, ap_id){
			...
			window.location.href = "{{url('dienstleister/kunden-details')}}/"+ap_id;
		}
