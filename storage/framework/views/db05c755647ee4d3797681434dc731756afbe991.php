<!-- Footer -->
<footer>
        <div class="container">
            <div class="top-footer">
                <div class="row">
                    <div class="col-xl-3 col-lg-3 col-md-4 col-4">
                        <h4>Quick Link</h4>
                        <ul class="footer-menu">
                            <li><a href="<?php echo e(URL::to('cosmetic-area')); ?>">Bereiche </a></li>
                            <li><a href="<?php echo e(URL::to('cosmetic-advantages')); ?>">Deine Vorteile</a></li>
                            <li><a href="<?php echo e(URL::to('become-partner')); ?>">Für Geschäftspartner</a></li>
                        </ul>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-4 col-4">
                        <h4>Support</h4>
                        <ul class="footer-menu">
                            <li><a href="javascript:void(0)">Kontakt </a></li>
                            <li><a href="javascript:void(0)">Hilfe </a></li>
                            <li><a href="javascript:void(0)">FAQ</a></li>
                        </ul>
                    </div>
                    <div class="col-xl-3 col-lg-2 col-md-4 col-4">
                        <h4>Rechtliches</h4>
                        <ul class="footer-menu">
                            <li><a href="<?php echo e(route('imprint')); ?>">Impressum </a></li>
                            <li><a href="<?php echo e(route('datenschutz')); ?>">Datenschutz</a></li>
                            <li><a href="<?php echo e(route('agb')); ?>">AGB</a></li>
                            <li><a href="javascript:void(0)">Verwendung von Cookies</a></li>
                        </ul>
                    </div>
                    <div class="col-xl-3 col-lg-4 language-row">
                        <h4>Sprachen</h4>
                        <!-- <ul class="language">
                            <li><a href="javascript:void(0)"><span><img src="<?php echo e(URL::to('storage/app/public/Frontassets/images/germany.svg')); ?>"
                                            alt=""></span> DE
                                </a></li>
                            <li><a href="javascript:void(0)"><span><img src="<?php echo e(URL::to('storage/app/public/Frontassets/images/usa-today.svg')); ?>"
                                            alt=""></span>EN</a> </li>
                            <li><a href="javascript:void(0)"><span><img src="<?php echo e(URL::to('storage/app/public/Frontassets/images/french-guiana.svg')); ?>"
                                            alt=""></span>FR</a></li>
                        </ul> -->
                        <div id="google_translate_element"></div>
                        <div class="footer-btn-wrap">
                            <a href="javascript:void(0)"><img src="<?php echo e(URL::to('storage/app/public/Frontassets/images/google-play.svg')); ?>" alt=""
                                    style="display :block!important"></a>
                            <a href="javascript:void(0)"><img src="<?php echo e(URL::to('storage/app/public/Frontassets/images/apple-play.svg')); ?>" alt=""></a>
                        </div>
                    </div>
                </div>
                <div class="f-logo">
                    <a href="index.php"><img src="<?php echo e(URL::to('storage/app/public/Frontassets/images/f-logo.png')); ?>" alt=""></a>
                </div>
                <ul class="social-media">
                    <li><a href="javascript:void(0)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path
                                    d="M12 9a3 3 0 1 0 0 6 3 3 0 0 0 0-6zm0-2a5 5 0 1 1 0 10 5 5 0 0 1 0-10zm6.5-.25a1.25 1.25 0 0 1-2.5 0 1.25 1.25 0 0 1 2.5 0zM12 4c-2.474 0-2.878.007-4.029.058-.784.037-1.31.142-1.798.332-.434.168-.747.369-1.08.703a2.89 2.89 0 0 0-.704 1.08c-.19.49-.295 1.015-.331 1.798C4.006 9.075 4 9.461 4 12c0 2.474.007 2.878.058 4.029.037.783.142 1.31.331 1.797.17.435.37.748.702 1.08.337.336.65.537 1.08.703.494.191 1.02.297 1.8.333C9.075 19.994 9.461 20 12 20c2.474 0 2.878-.007 4.029-.058.782-.037 1.309-.142 1.797-.331.433-.169.748-.37 1.08-.702.337-.337.538-.65.704-1.08.19-.493.296-1.02.332-1.8.052-1.104.058-1.49.058-4.029 0-2.474-.007-2.878-.058-4.029-.037-.782-.142-1.31-.332-1.798a2.911 2.911 0 0 0-.703-1.08 2.884 2.884 0 0 0-1.08-.704c-.49-.19-1.016-.295-1.798-.331C14.925 4.006 14.539 4 12 4zm0-2c2.717 0 3.056.01 4.122.06 1.065.05 1.79.217 2.428.465.66.254 1.216.598 1.772 1.153a4.908 4.908 0 0 1 1.153 1.772c.247.637.415 1.363.465 2.428.047 1.066.06 1.405.06 4.122 0 2.717-.01 3.056-.06 4.122-.05 1.065-.218 1.79-.465 2.428a4.883 4.883 0 0 1-1.153 1.772 4.915 4.915 0 0 1-1.772 1.153c-.637.247-1.363.415-2.428.465-1.066.047-1.405.06-4.122.06-2.717 0-3.056-.01-4.122-.06-1.065-.05-1.79-.218-2.428-.465a4.89 4.89 0 0 1-1.772-1.153 4.904 4.904 0 0 1-1.153-1.772c-.248-.637-.415-1.363-.465-2.428C2.013 15.056 2 14.717 2 12c0-2.717.01-3.056.06-4.122.05-1.066.217-1.79.465-2.428a4.88 4.88 0 0 1 1.153-1.772A4.897 4.897 0 0 1 5.45 2.525c.638-.248 1.362-.415 2.428-.465C8.944 2.013 9.283 2 12 2z" />
                            </svg></a></li>
                    <li><a href="javascript:void(0)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path
                                    d="M14 19h5V5H5v14h7v-5h-2v-2h2v-1.654c0-1.337.14-1.822.4-2.311A2.726 2.726 0 0 1 13.536 6.9c.382-.205.857-.328 1.687-.381.329-.021.755.005 1.278.08v1.9H16c-.917 0-1.296.043-1.522.164a.727.727 0 0 0-.314.314c-.12.226-.164.45-.164 1.368V12h2.5l-.5 2h-2v5zM4 3h16a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1z" />
                            </svg></a></li>
                    <li><a class="tiktok" href="javascript:void(0)"><svg enable-background="new 0 0 512 512"
                                viewBox="0 0 512 512">
                                <path
                                    d="m461 119c-54.5 0-98.9-44.2-98.9-98.5l-.1-20.5h-116v346.2c-.1 22-14.1 41.4-35 48.3-5.2 1.7-10.6 2.6-16.1 2.6-28.2 0-51.1-22.9-51.1-51.1 0-21.1 12.9-39.3 31.2-47.1 6.1-2.6 12.8-4 19.9-4 5.5 0 10.9.9 16.1 2.6l4 1.3v-117.6c-6.1-.7-12.3-1-18.4-1-7.3 0-14.5.5-21.6 1.4-36.1 4.7-69.5 21-95.7 47.2-31.3 31.3-48.5 72.9-48.5 117.2s17.2 86 48.5 117.3 73 48.6 117.3 48.6c8.3 0 16.7-.6 24.9-1.8 35.1-5.3 67.1-21.4 92.4-46.7 31.1-31.1 48.4-72.5 49.1-116.6v-136c4.4 2.5 9.5 4.9 14.6 7.1 26.5 11.2 83.7 17.1 83.7 17.1h19.7v-116zm-20 74.6c-16.6-1.9-32.7-6.1-48.2-12.6-13.8-5.8-26.7-13.4-38.5-22.4l-32.3-24.8v212.8c-.1 33.4-13.2 64.9-36.9 88.5-19.2 19.2-43.5 31.5-70.1 35.5-6.2.9-12.6 1.4-19 1.4-33.7 0-65.3-13.1-89.1-36.9s-36.9-55.4-36.9-89.1 13.1-65.3 36.9-89.1c18.7-18.8 42.4-30.9 68-35.2v36.9c-39.5 9.5-68.9 45-68.9 87.2 0 49.5 40.4 89.7 90 89.7 9.6 0 19.1-1.5 28.3-4.6 36.7-12.1 61.5-46.2 61.6-84.9v-306h37.5c8.8 60.5 56.9 108.4 117.5 117.2v36.4z" />
                            </svg></a></li>
                    <li><a href="javascript:void(0)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path
                                    d="M15.396 10.58l.02-.249a32.392 32.392 0 0 0 .083-2.326c0-.87-.294-1.486-.914-2.063-.66-.614-1.459-.942-2.59-.942-1.137 0-1.958.335-2.51.888-.696.695-.958 1.218-.958 2.1 0 .521.061 1.994.096 2.618a2 2 0 0 1-.469 1.402c.055.098.105.204.153.317.3.771.198 1.543-.152 2.271-.392.818-.731 1.393-1.41 2.154a7.973 7.973 0 0 1-.642.643 1.999 1.999 0 0 1 .412.565 5.886 5.886 0 0 1 1.585.074c.81.146 1.324.434 2.194 1.061l.016.011.213.152c.619.44.877.546 1.473.546.609 0 .91-.121 1.523-.552l.207-.146c.876-.632 1.407-.928 2.231-1.076a6.664 6.664 0 0 1 1.559-.074 1.999 1.999 0 0 1 .417-.567 8.409 8.409 0 0 1-.616-.616 9.235 9.235 0 0 1-1.447-2.16c-.363-.749-.47-1.54-.137-2.321.04-.098.085-.19.132-.276a2 2 0 0 1-.469-1.435zm-10.315-.102c.419 0 .6.305 1.219.305.157 0 .26-.035.326-.066-.009-.156-.099-1.986-.099-2.729 0-1.688.72-2.69 1.543-3.514C8.893 3.65 10.175 3 11.996 3c1.82 0 3.066.653 3.952 1.478.886.825 1.551 1.93 1.551 3.528 0 1.555-.099 2.594-.108 2.716a.59.59 0 0 0 .279.065c.63 0 .63-.31 1.33-.31.685 0 .983.57.983.823 0 .621-.833.967-1.33 1.126-.369.117-.931.291-1.075.635-.074.174-.043.4.092.678.003.008 1.26 2.883 3.93 3.326.235.035.391.241.391.483 0 .332-.37.617-.726.782-.443.2-1.091.37-1.952.505-.043.078-.134.485-.235.887-.135.542-.801.366-.991.326A4.997 4.997 0 0 0 16.291 20c-.482.087-.913.378-1.395.726-.713.504-1.465 1.076-2.9 1.076-1.436 0-2.144-.572-2.857-1.076-.482-.348-.905-.637-1.396-.726-.898-.163-1.57.036-1.795.057-.226.02-.842.244-.996-.327-.045-.166-.191-.808-.235-.895-.856-.135-1.508-.313-1.952-.513-.365-.165-.726-.443-.726-.779 0-.235.158-.44.391-.482 2.644-.483 3.766-3.005 3.922-3.33.132-.276.161-.5.091-.679-.143-.343-.704-.513-1.073-.635-.105-.034-1.336-.373-1.336-1.117 0-.24.205-.573.582-.73a1.36 1.36 0 0 1 .465-.092z" />
                            </svg></a></li>
                    <li><a href="javascript:void(0)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">

                                <path
                                    d="M4 3h16a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1zm1 2v14h14V5H5zm2.5 4a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm-1 1h2v7.5h-2V10zm5.5.43c.584-.565 1.266-.93 2-.93 2.071 0 3.5 1.679 3.5 3.75v4.25h-2v-4.25a1.75 1.75 0 0 0-3.5 0v4.25h-2V10h2v.43z" />
                            </svg></a></li>
                    <li><a href="javascript:void(0)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path
                                    d="M7.253 18.494l.724.423A7.953 7.953 0 0 0 12 20a8 8 0 1 0-8-8c0 1.436.377 2.813 1.084 4.024l.422.724-.653 2.401 2.4-.655zM2.004 22l1.352-4.968A9.954 9.954 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10a9.954 9.954 0 0 1-5.03-1.355L2.004 22zM8.391 7.308c.134-.01.269-.01.403-.004.054.004.108.01.162.016.159.018.334.115.393.249.298.676.588 1.357.868 2.04.062.152.025.347-.093.537a4.38 4.38 0 0 1-.263.372c-.113.145-.356.411-.356.411s-.099.118-.061.265c.014.056.06.137.102.205l.059.095c.256.427.6.86 1.02 1.268.12.116.237.235.363.346.468.413.998.75 1.57 1l.005.002c.085.037.128.057.252.11.062.026.126.049.191.066a.35.35 0 0 0 .367-.13c.724-.877.79-.934.796-.934v.002a.482.482 0 0 1 .378-.127c.06.004.121.015.177.04.531.243 1.4.622 1.4.622l.582.261c.098.047.187.158.19.265.004.067.01.175-.013.373-.032.259-.11.57-.188.733a1.155 1.155 0 0 1-.21.302 2.378 2.378 0 0 1-.33.288 3.71 3.71 0 0 1-.125.09 5.024 5.024 0 0 1-.383.22 1.99 1.99 0 0 1-.833.23c-.185.01-.37.024-.556.014-.008 0-.568-.087-.568-.087a9.448 9.448 0 0 1-3.84-2.046c-.226-.199-.435-.413-.649-.626-.89-.885-1.562-1.84-1.97-2.742A3.47 3.47 0 0 1 6.9 9.62a2.729 2.729 0 0 1 .564-1.68c.073-.094.142-.192.261-.305.127-.12.207-.184.294-.228a.961.961 0 0 1 .371-.1z" />
                            </svg></a></li>
                </ul>
            </div>
            <p class="copy-right">©2021 reserved4you GmbH </p>
        </div>
    </footer>
<?php /**PATH F:\projects\laravel\reserved4you_dev\resources\views/Includes/Front/footer.blade.php ENDPATH**/ ?>